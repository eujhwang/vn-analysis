...
def _create_dataset(args: Dict[str, Any], dataset_id: str):
    ...
    # print("data_edge_dict:", data_edge_dict) # dict["train", "valid", "test"]
    if args.train_idx and torch.cuda.is_available():
        print("Using", args.train_idx)
        _train_idx = pd.read_csv(os.path.join("../data", "{}_idx".format(dataset_id), args.train_idx + ".csv.gz"),
                                compression="gzip", header=None).values.T[0]
        data_edge_dict['train']['edge'] = data_edge_dict['train']['edge'][_train_idx]
        train_idx = [i * 2 for i in _train_idx] + [(i * 2) + 1 for i in _train_idx]
        data.edge_index = data.edge_index[:, train_idx] # we now have the edges shuffled because the second index is concatenated
