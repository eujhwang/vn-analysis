...
# code from https://github.com/JiaxuanYou/P-GNN
...
class PGNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, layer_num=2, dropout=True, feature_pre=True, **kwargs):
    ...
    def forward(self, data):
        ...
        if self.feature_pre:
            x = self.linear_pre(x)
        x_position, x = self.conv_first(x, data.dists_max.to(x.device), data.dists_argmax.to(x.device))
        if self.layer_num == 1:
            return x_position
...
class PGNN_LinkPredictor(torch.nn.Module):
    def __init__(self):
        super(PGNN_LinkPredictor, self).__init__()
