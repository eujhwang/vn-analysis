...
class RandomVirtualNode(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, num_nodes, num_virtual_nodes,
                 model, rand_num=1, aggregation="sum", activation="relu", JK="last", normalize=True, cached=False):
        super().__init__()
        self.num_layers = num_layers
        self.convs = torch.nn.ModuleList()
        self.batch_norms = torch.nn.ModuleList()
        ...
        self.vn_index = torch.zeros(num_virtual_nodes, num_nodes)
        for i in range(num_nodes):
            rand_indices = torch.randperm(num_virtual_nodes)[:self.rand_num]
            self.vn_index[rand_indices, i] = 1
        ...
        for layer in range(self.num_layers):
            ...
            if layer < self.num_layers-1:
                ...
                for v in range(self.num_virtual_nodes):
                    ...
                    virtual_node_tmp = global_add_pool(embs[layer][self.vn_index[v] == 1].squeeze(), torch.zeros(1, dtype=torch.int64, device=x.device))
